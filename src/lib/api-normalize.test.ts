...
import { normalizeApiHuvudmanList, normalizeApiSchool } from "./api-normalize";
...
describe("normalizeApiSchool", () => {
  ...
  test("non-contiguous coverage keeps its gap in the span", () => {
    // Years are rebuilt from the per-form lists, so that is where the gap
    // has to be expressed.
    const s = normalizeApiSchool(
      skola({
        årskurserPerSkolform: [
          { kod: "fsk", skolform: "Förskoleklass", årskurser: ["0"] },
          { kod: "gr", skolform: "Grundskola", årskurser: ["4", "5", "6"] },
        ],
      }),
    );
